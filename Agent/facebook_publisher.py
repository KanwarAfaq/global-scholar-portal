...
def publish_video(
    ...
    """
    Upload a local MP4 to the Facebook Page video endpoint.

    Returns the Facebook video ID. Facebook may continue processing the
    uploaded video for a short time after this call succeeds.
    """
    path = os.fspath(video_path)

    if not os.path.isfile(path):
        raise FileNotFoundError(f"Facebook video file not found: {path}")
    ...
    try:
        ...
        raise FacebookPublishError(
            f"Could not connect to Facebook while uploading video: {exc}"
        ) from exc
    ...
    if not video_id:
        raise FacebookPublishError(
            "Facebook reported video upload success but returned no video ID."
        )
